...
```text
Last completed: Slice 15 (双齐为基线 + 末行对齐选项，ADR 0023)
Up next:        Slice 16 (gap audit 缺口 7：行间线——专名号/书名号甲式，复用 decoration 通道) 或缺口 6（行尾悬挂 opt-in）；更远期：Android composable 渲染、竖排预研
```
