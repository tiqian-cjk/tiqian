...
| 21 | — | 中西混排西文音节连字：`tiqian-linebreak` `Hyphenator`/`LiangHyphenator` + 内置 en-US TeX 模式；引擎 `LineEndHangingHyphen` 拆音节 cluster、行尾悬挂连字符（不占版心） | `LiangHyphenatorTest`/`EnglishHyphenationTest`/`HyphenationLayoutTest`；`western-hyphenation` fixture golden | golden + `:tiqian-linebreak:jvmTest` + playground 目检 | done (默认 `NoHyphenator` 零漂移、注入 `enUs` 开启；源文本不动；ADR 0029) |
...
Slice 0–21 全部 `done`，第一阶段（CLREQ 简体横排）覆盖收口——gap audit 七缺口 + 自适应默认 + grid-first 整数行长 + 中西混排西文连字均落地。下一个大方向（竖排预研 / Android 真渲染）属第二阶段，见「不在第一阶段做的」。
