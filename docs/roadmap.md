...
之前:          **Web 端引擎跑通（第二阶段起步）**（2026-07-08，ADR 0039，Slice 34；2026-07-15 迁移到纯 Kotlin/JS layout runtime；2026-07-16 浏览器 exact-font 改为 server shaping replay）。`tiqian-shaping-web` 以离屏 Canvas 度量，`tiqian-web` 以预断行 DOM 消费统一 `LayoutResult`，断行、推入推出、邻行均摊、justify 与断词不交给浏览器；Edge 增强安全性实测促使浏览器删除 Kotlin/Wasm target，并进一步把 HarfBuzz / WOFF2 WASM 限定在 Node precompute，客户端 snapshot miss 只缩放服务器回放证据。
