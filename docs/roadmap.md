...
行间注 第一刀: **拼音 ruby**（ADR 0032）落地——`RubySpan(baseRange, text)` 进 `LayoutInput.rubySpans`，注文居中于基字上方、行高均匀预留注文带（CLREQ「行距不随标注与否变」）、基文进 `unbreakableRanges`（不可拆）、advance 不动（注文宽则**悬挂**，避让待后续）。作者面 `cjkRuby("北京","Běijīng")`（注文不进源）。`RubyDecisionInfo` 入 dump。无 ruby 时零漂移。**第二刀 注音**（右侧竖排 ㄅㄆㄇ+调号+半字预留，动 advance）= 下一个 ADR。
