...
```text
Last completed: **Compose API 收口**（2026-06-21，Codex 评审）——#1 `ColorSpan` 移入 core（公开签名不泄漏 Skia 类型）；#2 `CjkBlock.Paragraph`/`List` 承载 `AnnotatedString`（富文本进多段/列表，dogfood 阻塞解除）；#3 `measurer` 独占 profile 源（删冗余 `profile` 形参）；#6 raw-span 入口降 `CjkParagraphImpl/internal`；#4 `onTextLayout` 暴露 `LayoutResult`；measure 阶段 bug（非快照 holder + height 钳 maxHeight）。**未做（引擎阶段过早）**：#4 semantics/baseline、#5 全套 Compose-facing style（`ic` 已铺地基）
...
Up next:        在 compose 上**实际 dogfood** 整套（富文本 + 列表 + 拼音/注音 + `ic` 缩进 + 推入压缩）——#2 已让富文本进多段/列表，阻塞解除。把模型用稳再调旋钮。第二阶段（竖排 / web）等模型冻结、考虑 Rust core 后再起；西文连字零散后续：整段最优连字、Android 原生断词器接 `Hyphenator`
```
