...
| 6 | M5 | API 固化；`tiqian-shaping-android` / `tiqian-shaping-skia` 真 adapter；golden test + benchmark | 平台 fixture + screenshot golden | 各平台模块 build + screenshot 测试 | wip (6a: shaping contract + `ExplainableStubTextShaper` + debug decisions；6b: `tiqian-shaping-jvm` + `AwtTextShaper` + playground 默认 real advance + punctuation atom 消费 shaped glyph bounds + `MissingInkBoundsFallback` 缺失 bounds 留痕 + `tiqian-shaping-skia` `SkiaTextShaper` 与 AWT↔Skia 对照 golden；ADR 0008/0013/0014/0015；`halt`/`chws` 与 Android adapter 未接) |
