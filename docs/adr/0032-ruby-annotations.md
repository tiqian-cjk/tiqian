...
- **行高 + 垂直摆位（实测度量，非猜 em）**：取**注文字体在 rubyFontSize 下的真实
  `ascent/descent`**（字面框，ADR 0002 amendment「用字体声明度量」）+ 基字真实 ascent，把注文
  字面框**叠在基字字面顶上方**，额外 clearance `RUBY_STACK_GAP_EM` **默认 0**——字面框本身含
  墨迹边距（汉字墨迹不顶字面顶、西文降部不到 descent 底），贴齐即分开；要更松才加：
  - 行带 = `注文 ascent + descent (+ gap)`（每行均匀预留，CLREQ「行距不随标注与否变」）;
  - 注文基线下移 = `基字 ascent + 注文 descent (+ gap)` —— 注文降部（g/j/p/y）自然越过基字字面顶。
