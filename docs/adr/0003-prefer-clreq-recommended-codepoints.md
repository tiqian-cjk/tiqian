...
### Amendment (2026-07-15): ExactEllipsisDisplayCoverage
...
让公共 layout pipeline 复用 `SubstitutionRollbackOnMissingGlyph` 回到 U+2026；不得因此把
整个段落降级到 Canvas，也不得从不可验证的系统 fallback 借一个 U+22EF。禁则、source
range、复制、搜索和无障碍语义始终按原始 U+2026 处理。

没有 exact font session 的 Canvas backend 无法证明 glyph 来自当前正文 face；它把候选
