# ADR 0052: precompute 缓存分层、批量渲染器与二进制桥接协议
...
NoCache、Memory、Directory、SQLite 实现均未开工。两个接入站点（sveltekit 站点与 astro 站点）为此
各自维护宿主侧缓存：context 指纹计算与并发去重在两个站点重复出现，compact 序列化、
原子写、逐条失效只在其中一个站点实现。

缓存条目的内容构成可以量化。sveltekit 站点一次完整构建的缓存共 306 个条目、115.2 MB，逐字段
...
render、backend revision、shaping 引擎与版本、face 集合指纹、typography，另加桥接
...
失效按桶推进。宿主每次提供一个桶 id 与该桶现存的段落哈希列表；库在该桶文章行归属的
条目里删除不在列表中、也不被其他桶文章行引用的条目，并把该桶的文章哈希行裁剪到列表，
哈希集清空的文章行随之删除。跨桶共享的条目（公共模板段落、被多篇引用的同一段文字）
在单桶更新后存活。逐桶清完一遍后，存活集合与「全部现存文章的段落哈希并集」一致；桶
使单次调用只携带一个桶的哈希列表，不出现整库并集一次传输。无文章行归属的条目（含
结构升级前写入的旧条目）不在任何桶的范围内，由按 context 的整库 prune 清理。这取代
0050 原设计的逐条失效。
...
`JsAdapter` 为 Neon 对接，读与写都按批传输，没有计算过程中的重入回调：

- 读方向是 prefetch：JS 侧按文章索引把条目批量写入 Rust 内存层，排版命中内存层，
  不逐条跨桥。
- 写方向是写出队列：Rust 侧攒批，数组满或构建结束时一次交 JS 写入磁盘。
...
### `BinaryBridgeProtocol`：按类型分传的二进制桥接
...
字节，这是协议与缓存在同一批改动中实施的原因，避免两套格式与两次转换。两端的 JSON 保留：
引擎 plan 输出与缓存条目自身的序列化仍是 JSON，本协议只替换调用边界的整棵序列化。

哈希先行：身份已知的内容不再传输。哈希在宿主侧用运行时内建哈希函数计算（node:crypto，
Bun 与 Deno 均兼容，Bun 另有 CryptoHasher），算法 SHA-256，与协议既有的 sha256 一组字段
...
同样由宿主计算；文章哈希加 context 命中 Article 层时整篇结果一次取回，按段落的请求全部
省去。内容只在确实需要计算时传输，热缓存与重建路径需要传输的量是一组哈希。
...
1. **全站共享的表**：faces、typographies、valueStyles、fontPreloads、strings 行、probe 行、
   metrics 行、revision 常量。一组表由一次构建持有，构建内多个 precomputer 的行并入
   同一组；内容寻址，表内容的哈希即键。按附录测量，faces 全站 110.1 KB、probe 行
   602.3 KB、metrics 行折算 0.56 MB、strings 行折算 0.2 MB，纳入全站表后整组一份。
2. **单篇文章的 manifest**：entries 以整数索引引用全站表；条目的 fontFaceEvidence 不再内嵌
   coverageText 文本与 probe 对象，改为 face 引用加 probe 行引用，正文在条目对应的
   来源里已有。fontReplay 的 shapes 留在单篇文章内：去掉双份内嵌后，shape 行跨篇去重的余额约
   1.3 MB，按行内容寻址的块的复杂度不匹配收益，列为后续候选。manifest 携带所引各表的
   sha。
...
`renderSnapshotTemplate` 删除。HTML 预备流程在单次调用内部走同一条拆分路径，
返回结果对象与表文件字节。

按附录的字节构成重排，sveltekit 站点形态的缓存体积预计约 50 MB，为当前 115.2 MB 的 43%
左右；HTML 产物里内嵌的 manifest 同步变小。

### `TableTransport`：表经根属性按需加载

根元素以属性指向全站表的 URL（内容哈希文件名，属性名 `tq-tables`，多表以空格分隔）。
...
- 缓存从可选附件变为批量渲染器的组成部分：走批量渲染器即得缓存行为；`NoCache` 仍是
  可用配置。
- bundle schema 升到 2，读取端兼容一个版本；全站表文件是新的构建产物。
...
- **head 放置或引用前内联的表传输。** 流式 SSR 的到达顺序与动态 typography 的枚举
  困难；属性拉取把顺序问题变成按需等待。
- **全站单一表。** 动态内容无法预先枚举全部 precomputer；按 precomputer 内容寻址后
  没有全局完整性要求。
...
- **fontReplay 行跨篇合并。** 双份内嵌消除后，shape 行去重余额约 1.3 MB，字符串与
  metrics 已纳入全站表；按行内容寻址的块的追加与失效复杂度不匹配收益。
- **哈希集中在 Rust 计算。** 宿主首次遇到内容仍要传输全文，热缓存构建省不掉传输；
...
## 附录（2026-08-21）：sveltekit 站点构建缓存输出测量

对象为 sveltekit 站点一次完整构建的缓存目录：306 个 JSON 条目，磁盘 115,221,583 字节。字段
...
单份约 10.0 MB，字体证据出篇后忽略不计，呈现字段 22.4 MB，全站表约 0.8 MB，四项合计
...
桥接、TS 持久化 SDK（宿主只提供地址到字节的存储，记录字节不透明，命中经本地副本
摘要校验后使用，正文不传输）。验证由 crate 单元与集成测试、npm 测试及两侧 canonical
golden 向量承担。
...
不再固定引用旧哈希）；SDK 提供按文章记录哈希集合（`recordArticle`，快照与校验输入的
...
文章哈希行裁剪到列表、清空的文章行删除，单事务完成；SQLite 参考实现全部在库内
集合运算，keep 列表不经 JS 物化）与按并集的整库 prune（`pruneUnreferencedEntries`，
可先丢弃指定文章行；跨文章共享条目在单文章丢弃后存活由测试覆盖）。store 缺文章面、
缺 prune 或缺桶 prune 时按具名错误报告，宿主自带索引照旧。Rust trait 的 Article
面未实施：article key 是宿主概念，当前没有 Rust 宿主消费方。
...
## 附录（2026-08-21 第三批）：全站表与表传输的实施测量
...
sveltekit 站点的引擎缓存文件长到 937,504,768 B。分解：三个 context（6,192 条 447.9 MB、
848 条 40.8 MB、71 条约 0）都是当次构建仍在使用的，没有失效行；多余的是
...
- **`DeflatedStoreRecords`**：SDK 在 store 边界对整条记录做 deflate。外层是
  魔数 `TQZL` 加一个版本字节，解开后是原 `TQCR` 记录。选压缩，不重排 JSON，
...
- **存储维护入口**：`createSqliteCacheStore(path, { contexts })`。`contexts`
  声明该文件服务的全部 context（sveltekit 站点的三个 precomputer 会话共用一个文件，
  声明三个指纹；空列表不删任何行），打开时删去条目、文章行、哈希行里的其余
  context；随后读取空闲页计数，大于零才 VACUUM，没有声明时也执行这一步。
  VACUUM 拿不到锁时跳过，删除已经写入磁盘，下次打开再回收。`dropOtherContexts`
...
| 时点 | 文件字节 | 空闲页 | 行形态 |
| --- | --- | --- | --- |
| 改动前 | 937,504,768 | 106,823 | 7,111 行明文 |
| 改动后首次构建 | 208,920,576 | 23 | 5,184 行 TQZL + 1,927 行明文 |
| 随后一次构建 | 208,908,288 | 1 | 计数不变 |

按形态拆分：5,184 行压缩后合计 56,802,091 B（单条最大 42,228 B）；1,927 行
仍是明文 141,746,824 B（单条最大 242,894 B），这批行在该次构建中未被引擎排出，
保持原形态直到下次排出。主 context 从 447.9 MB 降到 187.7 MB。第二次构建全部
命中，行集合与形态计数不变，构建产出的 tiqian 面（表 sha、144 个 snapshot id）
与改动前的参照构建逐字节一致。

astro 站点：文件 74,588,160 B，两个 context（1,258 条与 9 条）。初始化维护删去
9 条的旧 context 并回收，文件 73,801,728 B，空闲页 1。这一次构建用了热缓存，没有排出写入，
