...
持久化写缓冲按数组攒批。数组上限由内存预算参数推导：宿主告知可用余量与保守程度，
缓冲不超过预算；缓冲满或构建结束时一次写出。
...
- 背压两个参数：队列深度与写缓冲内存预算。超限时入口等待，不无界排队。
...
引擎写缓冲上限 32 MiB（cache.rs 的 `DRAIN_QUEUE_MAX_BYTES`），超限的提交报
`CacheWriteBufferFull`。宿主侧两条使用规则使构建保持在上限以下：页面路径在每篇
文章渲染完成后调用一次 flush；预填按段落输入分块，每块等待结果并 flush。分块以
