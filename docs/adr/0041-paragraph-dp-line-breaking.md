...
- Status: **恢复执行·实验中**(2026-07-21,实际语料出现 lookahead 本可避免的可见
  字距拉伸,触发下方归档停用时写明的恢复执行条件,恢复候选对比；默认断行器不变,
  见文末恢复执行修订。此前状态:**冻结**,2026-07-18,三轮目检后归档停用：v3 代价下
  DP 与 lookahead 在实际语料上基本收敛,残余收益。极窄版心的极端场景保险与个别
  孤立改善。不足以支付转默认、约 3–10% 端到端性能与后续边模型投入)
- Date: 2026-07-17
- Refines: [ADR 0038](0038-neighbor-amortized-adjustment.md)(代价模型不变,本 ADR 换**求解器**)、
  [ADR 0031](0031-line-adjustment-direction.md)(「先挤压、后拉伸」不变,将推入从整体链处理改为解的内部变量)
...
`ParagraphDpReferenceExperiment`(jvmTest 参照实验:录制引擎喂给断行器的实际输入,
参照 DP 在同一输入、同一代价上求全局最优)量化了这个缺失项:

- 实际中文段 + 窄版心(240px ≈ 15 字/行)下,lookahead 留下最坏行密度
  **15.67px/gap** 的严重退化行,全局最优只需 1.60px/gap;
- 语料总代价保守下界(参照 DP 禁用连字符与推入边)已低 **43.8%**;
- lookahead 占优的行全部是绝对代价 < 1.5 的零散项,且源于参照实现宇宙较小
  (无悬挂/推入边),不是实际优势。

另一发现:ADR 0031 的填充操作连带链(`applyFillPushIn`)是逐边界贪心,
叠在全局规划上会把规划逐格改写回贪心形态。实测把 1.1 代价的 DP 方案
还原成 46 代价的 lookahead 输出。全局求解器必须将推入纳入解的求解范围,
不能留给事后链的后续处理。

## Decision

新增 `ParagraphDpLineBreaker`(`strategyName = "paragraph-dp"`,
`LineOptimizationStrategy.ParagraphDynamicProgramming` 已实现):
...
1. **`ParagraphGlobalAmortizedOptimum`** — 以行为边、断点为节点做精确 DP。
   ...
   加压缩边)。每段(mandatory break 分段)独立求解,密度跨段重置为 0,
...
2. **`CompressionAsDpEdge`** — 候选终点可越过自然装填点,只要超出量不超过
   ...
   commit 时压缩边经 `tryPushIn` 实现,repair 记录(分档顺序、行末削半晋升、
   allocations)与填充操作的连带链逐字节一致。本策略**不再运行** `withFillPushIn`。
3. **`KinsokuAvoidanceOverRepair`** — 存在有效替代方案时,直接过滤会把避头尾
   违规带入解的候选终点;不存在替代方案时保留violating 终点,由共享的
...
进度保证:DP 无最终状态时回退纯 greedy 链;基线 greedy 断点始终在候选集内,
...
- 语料总代价 **−30.3%**;严重退化行(>5px/gap)全部消失
  (narrow@240px:46.02 → 1.97;mourning-frame:240 → 60);
- 回归全部 ≤ 1.12(像素×gap, 标准度量, 视觉不可辨),
...
- 已知差距(后续工作项):悬挂边、连字符宽度记录进边模型、
  compressBias 对带符号密度的折价系数需随视觉 QA 校准。

## 修订(2026-07-18):目检否决转默认，缺陷在代价模型,不在求解器

实际博客语料 + 实际宽度(360px)并排目检(多名读者一致):lookahead 观感更好,
...
1. **`NaturalSetIsReference` 缺失**:邻行差项 `(dᵢ−dᵢ₋₁)²` 把「回到自然密排(d=0)」
   当成需要消除的突变,于是最优解把首行松量向后「平滑衰减」,造成松散扩散。
   自然密排是参考态:一行松挨一行自然不是缺陷,多行递松才是。凸项 d² 无此问题
   (严重退化行仍被正确定价),坏的只是邻行项的过零行为。
...
4. **`CompressionFirstFill` 缺失**(评审补充,2026-07-18):压缩边被定价成
   ...
   项重置为 0、只剩平坦罚分在错误方向上起效。方向偏好是引擎的策略旋钮,
   可见性是模型的度量,二者不可混用。

lookahead 的两行视野令它无法利用这些模型漏洞。此前 −30.3% 的「改进」在度量上成立,
...
- `NaturalSetIsReference`:邻行差项过零条件,仅当两侧都偏离自然密排时生效,
  压缩(负)贴拉伸(正)两侧非零、照罚。
- `GapClassBlindDensity` 修复:按 justify 实际渲染分层计价,中西 gap 先吸收
  差额(每个至 `sinoWesternStretchCap` 封顶),余量落 CJK 字距,逐类凸项;
  两类都吸不动的差值回线性 raggedness。
...
读者一致偏好孤立分布。根本原因：邻行差项奖励「相邻行拉伸相近」(diff≈0 免费),
...
持平。严重退化场景与 `CatastropheGuard` 全部保持。

### 归档停用结论(2026-07-18 三次目检)

v3 之后 DP 与 lookahead 在实际语料、实际宽度上基本收敛,残余差异不构成
可感收益。**这本身是本 ADR 最有价值的发现**:lookahead(窗口 2 + 两行
rollout)加上 ADR 0031 压缩优先与 ADR 0038 凸密度,已接近修正后感知模型
的最优;两行视野丢掉的全局收益集中在实际语料罕见的极窄严重退化场景。两天内
被全局最优暴露并证伪的五条模型信念(松散扩散、gap 类别盲、末行免检、
压缩定价倒置、邻行平滑目标本身)全部已记录。全局求解器作为「代价模型
的试验台」的价值,大于它作为默认断行器的价值。

## 恢复执行(2026-07-21):实际语料出现本可避免的可见拉伸

冻结三天后,实际阅读中观察到 lookahead 输出**本可避免的可见字距拉伸**。
即归档停用结论所称「实际语料罕见」的场景在日常语料中出现,命中恢复执行条件第一款。
...
lookahead;下一步是把观察到的具体段落纳入候选对比语料复现定位,再按数据决定
是走「悬挂边进 DP + 转默认」还是继续缩小 v3 模型的适用范围。归档停用结论的证伪链继续
有效:任何模型改动仍须过三轮目检同款的 DOM 尺子。

## 修订(2026-08-05):实验实现不进入发布产物,目检移入 layout report

归档停用后恢复的 web `lineBreakStrategy` 选项与 demo 对照区,把一个仍在调优、
...
- 目检区移入 `:layout:generateLayoutReport`:同一批实际博客段落,240px 与
