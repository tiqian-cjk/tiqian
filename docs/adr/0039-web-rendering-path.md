...
并让 CSS 在 runtime 前已经给出最终列表几何。构建期 snapshot 缓存可证明的 `<p>`；root 内存在需
runtime 接管的 `li` 或未预排正文时不采用部分快照，而由 runtime 按视口优先逐段接管。marker 与两字缩进
始终由同一份静态 Web CSS 持有。复杂 item 不能
...
`ResponsiveNativeBacking` 要求跨字格后在 resize task 内先 destroy 当前固定断行 DOM，恢复整 root 的
semantic source。它会立即按宿主 CSS 响应新宽度，避免 912px 的旧 Tiqian 行在 452px 容器里横向溢出。
