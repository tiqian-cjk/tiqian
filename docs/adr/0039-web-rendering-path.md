...
第二阶段要给提椠加 web 前端。一个需要留意的误判是「web 没有 Compose 那些平台能力缺陷
...
用同一条实际 pipeline 对 24 段正文做三轮 A/B：Kotlin/JS 每轮 299.1 / 308.9 / 307.6 ms，
...
字体，并把当前 pipeline 实际请求过的 segment shaping 与 font metrics 规范化成字号无关回放表。
...
### `HarfBuzzVerifiedCjkDash` : 破折号必须拿实际 face / glyph 证明
...
这既保持 exact 校验不变，也避免 JIT 把单篇文章验证放大成重复的主线程长任务。
...
集合，不使用会随同站路由变化的 `document.baseURI`。
...
视觉中线相差不超过具名容差；成对 U+2014 还要求各自约 `1ic` 且接合处无正空隙。默认 shaping 与
显式 `locl=0` 比对：确有替换才记 `LocalizedVariant`，否则记
...
`ExactGlyphReplayUnavailableOnDom` 的诚实边界，该边界不允许 DOM 重新决定候选。
...
裁掉行盒外侧来“碰巧”得到半宽。节点数 ≈ 行数 + 稀疏语义 span，远少于每字一个 span。
...
`LineMetricStrut` 的 `data-tq-line-width` 表示 layout advance，与 ink clip 无关。
...
因此 `styles.css` 是 DOM replay 的显式 capability，属于必需外观。npm command API 与 custom
...
`DecorationKind.Emphasis`，并把这些 grapheme 的字重恢复到进入 `<strong>` 之前所在的父元素字重；汉字由
既有 `EmphasisDotOnHanText` 逐字加点，中文标点按 CLREQ 跳过。西文、数字、emoji 不进入该
decoration，继续使用宿主 `<strong>` 的实际粗体，不能被 `BilingualEmphasisWesternItalic` 误改成
...
`InlineBoxBoundaryAdvance` 把宿主 inline 的实际边界几何作为 layout input，在渲染前获取。DOM
...
`box-decoration-break` 都只存在于一个实际元素上，不再需要 continuation clone、open-edge attribute
...
宿主入场动画在结束时仍保留 `translateY(0)`，以及只移动整个正文的平移帧；这些状态不改变段内 advance、
...
`link[data-tiqian-stylesheet]`，不能把一次已 resolve 的模块层 Promise 当成 stylesheet 永久
...
规范化为单个 LF；只有实际 `<br>` 另记为结构性 mandatory break。
投影同时生成 UTF-16 boundary map，`TextSpan`、decoration、inline box/object 与 source semantic range
必须一起重映射，不能只改字符串再猜 range。复制语义与宿主原生 `innerText` 比对：HTML 格式化
空白不凭空进入复制，实际 `<br>` 仍复制为一个换行；原 SSR children 在 destroy 后逐节点还原。
...
CSS `hyphens` 始终为 `manual`(即不自动断词)。断词开不开、用哪套词典,是引擎默认
(ADR 0029,当前默认开)的事，与 web 后端无关。web 只负责按引擎的断词结果绘制。

### `ReflowByRebreak` + `WidthIndependentAnnotationCache` : resize 只重跑折行

放弃「浏览器免费 reflow」，换成 `ResizeObserver` 驱动引擎重排。目标仍是只重跑折行那一趟：
cluster advance、locl 字形、基础 autospace、标点原子与几何记录等宽度无关量由引擎内部的
...
仍按 Kotlin `Float` 语义判断，不能用固定 `0.5px` 容差吞掉一次实际跨格。最大版心快照也按相同
...
一旦进入引擎生成几何的路径，`EngineOwnedPunctuationSpacing` 要求接管后的 flow 与 shaping
boundary 显式使用 `text-spacing-trim: space-all`。否则支持 `normal` 上下文标点压缩的浏览器会
...
文档层 handler 只在 selection 与 `[data-tq-rendered]` 相交时接管；站内其它文本必须继续走
浏览器原生 copy。`SourceFaithfulSemanticClipboard` 对接管的 selection 同时写入两种 payload：

- `text/plain` 按宿主 block 边界加入段间换行，保留实际 `<br>`，删除软折行和仅用于绘制的连字符；
...
- **Houdini CSS Layout API**:理论最优，把断行器注册进浏览器布局树,推入推出跑在**原生 reflow
  内部**,引擎拥有算法、浏览器驱动重排。但 Chromium-only、基本已废弃，不能做主路径。仅作渐进
  增强候选,不阻塞主设计。
...
`box-decoration-break`；默认 `slice` 因而在延续侧形成方角，首尾仍由宿主的圆角样式决定。
这与 Compose 的 1 dp 延续圆角在实现上不同：Web 保留 hover/focus、伪元素、border、padding
...
1. **`AtomicParagraphDomSwap`**：renderer 先把全部行盒构建进 `DocumentFragment`，再对宿主
   ...
   新行盒未接」的未受保护的 DOM 闪变窗口。异常路径的 rollback 仍逐节点恢复；该路径执行频率低，
   不计入 mutation 预算。
...
3. **`StaleFinishKeepsCommittedParagraphs`**：job 以 stale 收尾时不回滚已提交段落。
   逐 item 提交守卫已保证每个实施完成的段落与当时的实时量化 measure 一致；把整批回滚到
...
6. **`ClockTierDiscipline`**：帧预算 deadline 读 `performance.now`。rAF 回调参数是帧起点
   ...
   截止时间戳，两条时钟在构造凭证时对齐一次。200ms 防抖到期时间与时长统计同用
   `Date.now`，毫秒精度足够。
...
2. relayout job 以 `captureSignatures: false` 派发，`#layoutWorkMeasureSignature` 为空串，
   live 签名与空串比较永远为真，`layoutInputsChangedDuringWork` 的判定不需要 live 读数。
...
给已挂载的区块授予排版时间段。每次授予一张凭证（一个 GrantController 对象：收件人、
job generation、换算到 `Date.now` 域的截止时间戳、段数配额），排版循环每排完一段问
一次准入，至少排一段。

初版在此之上还维护一个全页共享的「排版时间段耗时估计」（滑动平均）：授予凭证前先按估计值
...
1. **调度者与被调度的工作在同一条线程。** 排版计算、DOM 提交、布局量读取、浏览器
   浏览器重排、coordinator 的帧循环全部在主线程。coordinator 用帧预算分时，但分时者自己
   也在被分时的线程里：宿主脚本的长任务会挤掉帧循环，被挤之后 rAF 回调参数（帧
   起点时间戳）在实际执行时早已落后，就是这条链的实例（`ClockTierDiscipline`
...
2. **取消与预算的最小作用单位是整个段落。** 排版作业按段落推进：断行、准备、
   ...
   实测短段不足 1ms，首次 enhance 的长段 5 到 20ms，单个超重段落可以耗尽整帧
   预算，帧的截止时间拦不住进行中的那一段。
   ...
   毫秒，不足一毫秒的剩余时间可放行大量廉价段落，配额按段数封顶。job generation 在
...
3. **每条停止路径必须带上重派义务。** 协议要求：任何取消、挂起、强制终止排版工作的
   ...
   收尾路径与回屏分支都会消费它；离屏强制终止在回到视口时清除挂起并按标志重排；帧
   任务挂起 200ms 到期由计时器整桶放行；排版凭证的发放挂起有独立的唤醒计时器，到点
   重启帧循环；两种挂起的唤醒都不依赖新输入到达。断连与禁用的沉默有意。曾发生的缺陷：
...
原生后续（a11y 记录与 style/layout flush）在 JS 归还后同一个任务内结算，凭证的
截止时间只约束 JS 执行时间段时长，约束不了一段凭证放行多少段提交、带出多少原生后续。
...
信号选帧距而非执行时间段 JS 耗时，因为原生后续的成本不在我们的 JS 栈下，帧距是唯一
覆盖全记录的测量，代价是判决晚一帧生效。与上午拆除的执行时间段耗时估计层有三点区别：
