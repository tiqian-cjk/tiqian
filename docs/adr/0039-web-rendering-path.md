...
`all: unset !important` 清掉宿主对通用 `span` / `svg` 的盒模型，再用 inline-important 写回引擎
拥有的尺寸与位置。source semantic clone 不做这种 reset，继续让宿主 selector、伪类与 transition
生效。flow 所需的 white-space / wrapping longhand 逐项记录所有权；destroy 只在属性仍等于
Tiqian 写入值时恢复原值，宿主运行期改动不能被无条件回滚。
...
`CjkStrongAsEmphasisMark` 是其中唯一由 HTML 语义触发的中文排版映射：`<strong>` 内被同一
`FontRoleClassifier` 判为 `CjkText` / `CjkPunctuation` 的 grapheme 降为
...
发布形态是 ESM package `@tiqian/web`：`<tiqian-prose>` 使用 light DOM，按需加载优化后的 Kotlin
...
浏览器原生 copy，不能因为安装 Tiqian 而被全局降成 plain text。
