...
并让 CSS 在 Wasm 前已经给出最终列表几何。v1 构建期 snapshot 仍只缓存 plain `<p>`；命中时仅为
...
发布形态是 ESM package `@tiqian/prose`：`<tiqian-prose>` 使用 light DOM，按需加载优化后的 Kotlin
Wasm runtime。custom element 的 connected / disconnected 生命周期负责 enhance / destroy，
因此 Astro / Swup 等宿主不需要理解 Tiqian 内部状态或猜构建产物 hash。发布 allowlist 必须包含
Gradle 生成的 `runtime/*.mjs` 与 `.wasm`；仓库内 build helper 不是安装后可执行的 CLI，不能以一个
...
Wasm runtime 内部的 `TiqianWeb.enhance()` 在 runtime 已安装后同步完成一个 root；ESM package 的
...
单 slice 耗时，避免只看 wasm 下载大小而忽略 main-thread long task。
...
恢复增强。ESM 入口的字体/Wasm 异步准备另由 `AsyncPreparationCancellation` 代际化：`destroy()`
...
`MandatoryBreak` 的 `<br>` 保留可访问的换行语义。动态 Wasm DOM 与构建期 canonical DOM 必须
