...
- Status: Accepted + Implemented (Slice 34-35; 2026-07-11 host integration amendments;
  ...
  2026-07-15 Kotlin/JS-only runtime amendment)
- Date: 2026-07-07
...
这里删除的是 Kotlin/Wasm layout backend，不是声称依赖树中绝对没有 WebAssembly。可选的 exact-font /
破折号字体证据仍懒加载 `harfbuzzjs`，Node 预排也用它；其 HarfBuzz core 自身是小型 WebAssembly，
但不承载断行、推入推出、行调整或 DOM render。普通 host-font / Canvas 路径与命中 snapshot 的首屏
不加载它，上述 Edge A/B 也测的是这条普通路径。若未来要求端到端零 WebAssembly，需要为
HarfBuzz 另做 backend，不能把 exact-font 证据静默删掉。Kotlin 的 `@JsFun` 目前仍使用名为
`ExperimentalWasmJsInterop` 的编译器 opt-in；该注解名称不是 WebAssembly target 或运行时依赖。
...
`OffscreenMeasureTextShaping`，只有实际含 `——` / `⸺` 的 root 才懒加载这条字体预检。

`CssomFontSourceResolution` 从当前段落的 computed CJK family / weight / style 出发，遍历
CSSOM 中同名 `@font-face`，按 `unicode-range` 找到破折号与“一”字各自实际会命中的 source。
family 名直接来自 CSSOM，包括 Astro 等构建器生成的名字；不得在库中猜 hash。只有可 fetch
的 URL source 才能验证，`local()` 或不可读的跨域 stylesheet 不能冒充已知 face。WOFF2 先还原
为 SFNT，再交给 HarfBuzz；HarfBuzz 明确使用 `script=Hani`、`language=zh-Hans` 和当前
`wght` variation，产出 glyph id / placement / advance / ink bounds。
...
异步预检建立只读 shaping session，layout 期间的 Web shaper 同步消费该 session；没有 session、
没有可验证 source 或所有候选不合格时返回 `NoConformingCjkDashGlyph`，该段保留原生 DOM，不能再
退回 Canvas 后宣称“中文变体已确认”。DOM 正文仍不能按 glyph id 重放，否则会损失选择/无障碍；
它把该 run 固定到 CSSOM 解析出的 exact family 并设置 `lang=zh-Hans`，随后以可见 Range 宽度与
HarfBuzz advance 交叉验证。不一致报告 `DomDashFaceGeometryMismatch` 并撤回增强。这是
...
并让 CSS 在 runtime 前已经给出最终列表几何。v1 构建期 snapshot 仍只缓存 plain `<p>`；命中时仅为
`li` 启动 list-only runtime，marker 与两字缩进始终由同一份静态 Web CSS 持有。复杂 item 不能
...
v1 最大版心 snapshot 只预排纯文本，不能证明这项富文本映射已经发生。root 显式开启转换且实际
包含 `<strong>` 时，`OptInStrongSnapshotExclusion` 跳过 snapshot adoption 并进入 browser runtime；
默认粗体路径仍可照常采用纯文本快照，不能让一个默认关闭的策略损失首帧能力。
...
### `ProgressiveParagraphEnhancement` —— SSR 先可用，排版分帧接管
...
候选段落在约 8ms 的 animation-frame budget 内分批处理，尚未处理的段落继续显示原始 SSR DOM；
断开 root 会取消待执行 frame，并精确还原已经增强的 children。完成事件同时报告总耗时与最大
单 slice 耗时，避免只看 bundle 下载大小而忽略 main-thread long task。

每段接管必须满足 `ParagraphAtomicNativeRollback`：lowering / layout / DOM render 任一阶段抛出
非预期异常，都先恢复该段原 children 与宿主属性，再发布具名 capability issue，并继续处理其它段；
渐进增强不能用空段或半成品 DOM 换取错误日志。
...
dispatch 时为准；段落分帧 lower 期间若响应式 CSS 刚好稳定，完成值才是引擎
...
仍按 Kotlin `Float` 语义判断，不能用固定 `0.5px` 容差吞掉一次真实跨格。最大版心快照仍以实际
content width 做严格匹配，字号、字体、行高等 typography 变化也必须重新 lower，不能被字格合并。
...
每一个中间宽度。若输入安静约 32ms 而在途 job 仍针对旧字格，`QuietLatestRetarget` 会取消尚未提交的
off-DOM 准备，并通过同一个 coordinator 直接改算最新 target；这不是重新引入首次跨格前的 debounce。
最大版心 snapshot 的精确宽度边界与 typography signature 仍是独立 invalidation，不能因为有效字格
相同而跳过。

`AtomicRelayoutCommit` 把 shaping / layout 放在约 8ms budget 的 animation-frame slices 中，只在全部
准备完成后于一个 frame callback 内替换整个 root 的 live paragraph DOM；任一 commit 异常在该
callback 返回前
恢复原 rendered 节点与宿主属性。连续拖动时采用 `BoundedResponsiveCatchUp`：设最后已提交的有效行长
为 `committed`、commit 前最新 live 行长为 `current`、job 准备出的行长为 `prepared`，只有
`prepared == current`，或 `prepared` 位于 `committed` 到 `current` 的闭区间内、确实推进了已提交状态，
并且离 `current` 最多一个字号字格时，才允许原子提交。这样不会把离手势多个字格的历史结果逐级播放；
拖动反向、越过 target、重复提交旧 measure 或不足一格时的非精确结果都直接丢弃。job 完成后若尚未
命中 latest，下一 animation frame 以 latest target 启动唯一 follow-up；若拖动已经安静，前述
`QuietLatestRetarget` 会更早取消旧准备。输入最终稳定后，状态必须收敛到最终 latest 行长；中途允许新
提交的 stale 结果最多落后一个字格，但旧 rendered DOM 在下一次原子 commit 前可能离 live width 更远，
不能把这项限制描述成「可见排版始终只落后一格」。整个过程不得恢复浏览器自行折行，也不能在多个
paint 中出现同一 root 上「部分段落新宽、部分段落旧宽」。

snapshot、runtime 与 native source 之间采用 `AtomicSnapshotNativeTransition`。异步加载 runtime、字体、
重新 lower 或准备 fallback 时，当前 snapshot / rendered DOM 始终留在 live tree；成功路径只允许在
同一个同步 event task 内恢复 backing source 并完成 atomic enhance，浏览器不能在两者之间 paint。
当前实现还没有 detached whole-root staging，不能把同步重建写成已完成的分帧准备。snapshot 验证失败、
离开最大版心，以及命名为 `InlineCloneDecorationBreakUnsupported` 的宽度相关 capability retry 都遵守
同一可见性规则：不得先 `destroy` 已增强正文再跨帧渐进重建。`InvalidWebShapingAdvance` 等稳定 issue
继续让对应段落保持 native，只重排其余已增强段落，不能因为一个 issue 重建整篇正文。若 capability 的
最终决策确实是 native fallback，可以在原子 commit 中把对应段落切到 native source，但其他可增强
段落不得经历一次临时的浏览器排版。

若在途 job 准备期间 typography attribute 或相关 FontFace 发生变化，
`ResponsiveTypographyCommitCancellation` 使旧 typography 结果失去提交资格，但仍保留当前 rendered
DOM；新 signature 作为 latest target 重新 lower。当前为避免跨 paint 暴露 native source，会在一个
同步 event task 内 atomic rebuild，并把整次耗时如实计入 `maxSliceMs`；这以可能出现一次 long task
换取不闪回原生排版。后续若接入 detached whole-root staging，才可以同时获得分帧准备与原子切换。
...
- SSR 正文始终先可见；客户端按段渐进接管，宿主 CSS / SEO / Pagefind / no-JS 路径不需要为
  Tiqian 重写一份。
