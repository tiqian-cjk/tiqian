...
replay model，不让 exact evidence 随文章段落数重复膨胀。

snapshot 未命中并进入 browser Kotlin/JS runtime 时，loader fetch 同一内容寻址 URL，并校验 source hash，
同时复核 face index、descriptor、axis tags、OpenType local names、构建时 `sourceOrder`、HarfBuzz version
与 backend revision；浏览器不再解析或 shaping 这些字体字节。纯 JS replay backend 按实时字号缩放
服务器结果，再由既有 `HarfBuzzSessionTextShaper` / `HarfBuzzSessionFontMetricsResolver` 适配给同一个
layout core。因此 miss 后重排仍不换回 Canvas 度量，却不在浏览器启动 HarfBuzz / WOFF2 WASM。
输入 key 缺失、字体证据失效或 live paragraph 超出服务器捕获的受控语义能力域时，具名回到
ADR 0039 的普通 browser adapter。
...
采用通过验证的 keyed candidate set；同 root 的 unkeyed candidate 保留 native source，由 runtime 按视口
优先逐段原子补齐。任一 keyed candidate miss 都不触碰正文，runtime 从完整 native source 开始接管。
...
写入 snapshot 级 `renderFontFamilies`。Node HarfBuzz、browser server-replay session、prepared SSR 与
...
browser replay session 并发取得 manifest 中互不相同的 face source并校验 source hash，随后安装上述
...
服务端已经直接写入 prepared DOM 的首屏 transport 不能配合
`font-display: optional|swap|fallback`：这些值允许首次绘制另一张脸，使首屏 DOM 失去“测量与绘制同源”
的前提；这一路径只接受默认 `auto` 或显式 `block`。响应式 SSR 的 inert snapshot 不在字体就绪前接管
正文，因此可以保留宿主的 display policy，但采用前必须让 `FontFaceSet.load()` 成功，并完成同一份
exact source / advance probe；加载失败时保持 native source。CJK unicode-range 可能让长文触达数十个 face；bundle 只把首个 prepared paragraph 的
...
- snapshot miss 后的 browser Kotlin/JS runtime 与 SSR 共用服务器生成的 HarfBuzz/OpenType shaping、metrics、
