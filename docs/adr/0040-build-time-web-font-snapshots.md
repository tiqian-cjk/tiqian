...
- Status: Accepted
- Date: 2026-07-11
- Amended: 2026-07-14（SSR / browser Wasm 共用 exact-font session、render-font family contract
  与 canonical DOM plan；role-aware 弯引号 shaping、shared prepared CSS、compact transport、
  direct first-paint bundle 与 client-navigation registration）
...
此时每次导航都重新下载 Kotlin Wasm、重新度量同一批字形并重跑最大宽度布局，是可以避免的工作。
...
新增环境无关的 HarfBuzz font session backend 与独立 Node Kotlin/Wasm 入口。Node wrapper 只负责读取
明确声明的 WOFF2 / SFNT 文件；共享 backend 负责把 WOFF2 还原为 SFNT、计算 source 与 SFNT
SHA-256、选择 face / unicode-range / variation，并建立只读 session。Kotlin/Wasm 继续调用
`ParagraphLayoutEngine`，不复制断行、标点 glue、推入推出或 justify 规则。

snapshot 未命中并进入 browser Wasm 时，只要 manifest 的 exact-font evidence 仍可用，浏览器 loader
...
垂直度量或另一套 shaping。只有字体 session 本身不可建立，或 live paragraph 已超出 plain snapshot
...
`font-feature-settings: "palt" 1` 重放。字体 advance probe、snapshot adoption 和 Wasm geometry gate 都把该 signature 纳入
...
Wasm 重排都不能为了逐 cluster 记录证据而退回每字一个 `inline-block`，选区必须保持连续。
...
构建产物 `PreparedParagraphV1` 包含 schema / engine revision、source digest、typography contract、
exact font evidence、最大 layout width 与由 `LayoutResult` 降出的 line/run render plan。第一版只接收
显式纯文本（含 source-faithful mandatory break）；rich semantic inline、ruby / 注音、opaque inline
object、伪元素盒、未知 emoji fallback 与破折号 exact-face 特例仍走客户端 pipeline。为与 core
...
固定最大版心站点可显式选用 `renderSnapshotBundle()` 的 `DirectFirstPaintPreparedDom` transport：
server adapter 只在最终 HTML response 中把 compact `entries` 写入 keyed `<p>`，同时注入
`initialStyle`、首个 prepared paragraph 实际使用字体的 preload 与 manifest-only template，并给 root 标记
`data-tq-ssr-snapshot` 和 `data-tiqian-exact-render-font`。页面数据和客户端导航 payload 仍保留原
source HTML；bundle 的 `clientTemplate` 可由浏览器入口在创建新 root 前注册 compact manifest，
客户端随后走同一 exact-font Wasm 路径，不把整篇 prepared DOM 再复制进 page data。
浏览器验证 live artifact digest 后直接采用现有节点，不再替换
一次相同 DOM；宽度 miss 时则从 canonical prepared DOM 的 `data-tq-src` / mandatory-break 契约重建
纯文本输入，geometry span 不得被当成宿主 inline object。两种 transport 都禁止用 `innerHTML`
反序列化来冒充原节点恢复。
...
browser Wasm canonical DOM 都使用这个投影；不能继续让前两者按内部 alias 测量、后两者却继承宿主
...
direct first paint 不能配合 `font-display: optional|swap|fallback`：这些值允许浏览器先画或永久保留
另一张脸，使 SSR 已经失去“测量与绘制同源”的前提。exact render alias 只接受默认 `auto` 或显式
...
“同名字体不同版本不一定字节相同”的事实，
不再宣称浏览器一定绘制构建时的 exact bytes。Node 与 browser Wasm 的**布局计算**始终使用 manifest
URL 的 exact bytes；同一浏览器中的 snapshot 与 Wasm 则显式使用 manifest 的同一个 render family
...
快照采用是 root 级 all-or-nothing，且只在以下证据全部匹配时发生：
...
7. clone 后（direct transport 为 live SSR artifact）每个显式 geometry / feature run 的实际 advance / `drawX`、每行 sentinel pen 与 core line
...
任一条件不满足时，DOM 保持或原子恢复 SSR，然后懒加载 browser Wasm。snapshot-backed 的 plain
...
Wasm canonical DOM 生成后再走与 snapshot adoption 相同的 segment、line pen、baseline 与 paragraph
height gate；只有这一步失配，才清除 canonical 标记并在同次调用内回到 ADR 0039 的 browser adapter。
session 证据失效或内容超出能力域时同样走该 adapter。最大宽度命中时仍只安装轻量 copy / restore /
observer 逻辑，不加载 HarfBuzz 或 Kotlin Wasm。
...
最大宽度变窄、响应式字号变化、字体换源或 probe 变化时，默认 transport 先原子恢复 SSR source；
direct transport 直接从 canonical prepared source lower；
...
复制 handler 从 Kotlin/Wasm 安装逻辑抽到独立小 JS；零 Wasm 快照路径与现有 runtime 共用同一
`data-tq-copy-ignore` / `data-tq-src` 契约，软折行不进入复制，mandatory break 仍保留源码换行。

## Consequences

- 常见桌面最大版心可以直接采用已排好的可选择 DOM，不下载或初始化 browser Wasm。
- snapshot miss 后的 browser Wasm 与 SSR 共用 exact font bytes、HarfBuzz/OpenType metrics、
...
- 同一字体的已安装本地版本可复用快照，但它属于显式的 compatible-local policy；exact-byte 需求仍应
  使用 URL-only source。
- 小屏、响应式排版、系统 fallback 与复杂 inline 内容继续使用 ADR 0039 路径；快路径不会降低
  既有能力。
...
- **无 manifest、无 canonical source 契约地把预断行 HTML 当 SSR 正文**：污染 no-JS / Pagefind /
  copy，响应式失效后没有可信 source。否。`DirectFirstPaintPreparedDom` 仅是有 digest、首帧 CSS、
  canonical source 与普通 client-navigation source backing 的显式 transport。
