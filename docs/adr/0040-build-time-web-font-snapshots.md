...
- Amended: 2026-07-14（SSR / browser runtime 共用 exact-font session、render-font family contract
  ...
  `font-variant-numeric: lining-nums` 的 `lnum` shaping / draw contract）
...
宿主显式声明 `font-variant-numeric: lining-nums` 时，构建和 browser exact session 都把它降为
HarfBuzz `lnum=1` 的段落基准 feature；字体 probe 记录实际启用的 `lnum`，live typography 必须仍为
...
响应式 runtime 可以在相同 family / weight / style / feature contract 下复用已验证的 exact 字体字节，
