...
空白不可逆。但 CjkInterChar（剩余字符间距）末档对**标点两侧一视同仁**——
glue 侧与**实心侧（括号内侧、点号前）都按与其他位置完全相同的均匀份额参与**。
CLREQ「剩余所有字符间距同时同等量拉伸」只排除两类：不可断标点规定的字间距、
连接号/分隔号及其前后字符；并未排除标点实心侧。早期「优先利用标点 glue」的
取舍已被 ADR 0004 amendment 废止。挤压（PushIn 等）另有独立的优先顺序，见
