...
> [!NOTE]
> 本文写于项目初始阶段，保留当时的目标、假设与 M1–M5 计划作为历史记录。
> 其中“暂不支持”“后续实现”等状态描述不代表当前实现。当前 pipeline 与模块边界见
> [architecture.md](architecture.md)，当前工作见 [roadmap.md](roadmap.md)，已确定取舍见
> [ADR 索引](adr/README.md)。

本文记录「提椠」（Tiqian）中文排版引擎最初的设计方向。它不是当前 API 或能力清单，
而是项目早期的取舍、核心模型和实现路线图。
