...
按当前 API 与已完成工作项复核。范围为简体中文横排。各“缺失项”小节保留发现问题时的描述，
...
- **着重号**：码点（●/•）、横排字底端、点号引号括号不加点、与字符居中对齐
  （疏排仍居中）。Slice 8 字面净空默认 0.1em，锚点由实际字面底边与圆点半径求得。
- **行间标点的行距下限**（5.6.1.1）：屏幕前端使用单面装 ≥1/2 字号的
  `InterlinearMarkLineSpacingFloor`。双面装 5/8 属于印刷透印约束，原
  `ParagraphStyle.printingSides` 已删除，待打印后端建模时再纳入（ADR 0018 amendment）。
- **行尾点号悬挂为非默认**：「绝大多数的中文出版物没有悬挂行尾点号的惯例」。
  ADR 0006 opt-in 立场正确（Slice 17 已实现，见缺失项 6）。
- **行首行尾禁则四档**（2026-06-13 补审，初版 audit 漏列）：「可以分为四种
  ...
  14–24 字为基本处理，>24 字为 GB 法，>32 字为严格处理，<14 字另启顿逗句悬挂。逐档严格限定分隔号行尾、
  破折号省略号行首；CLREQ 明示「行首行尾禁则规定属于排版风格……可以选择
  或者自定义」，故被实现为 `ClreqProfile.kinsokuMode`（`Fixed` 固定一档 /
...
Replace 模式（有 typed space 时替换为 gap），`AutoSpaceMode.Insert` 还是存在但不生效的枚举；
这是当时影响所有无空格混排文本的正确性缺失项。

当前默认的 `AutoSpacePolicy` 已对汉字与西文字母、数字边界使用
`AutoSpaceMode.Insert`，并以 `TextAutoSpaceInsert` 记录决定。

### 2. 西文词距（断行 + 调整）。已解决（Slice 12，ADR 0019；挤压档随缺失项 4）
...
只禁止在别处硬拆。被实现为 `linebreak` 的 `Hyphenator` 接口
...
### 3. 拉伸优先顺序与 CLREQ 有出入。已解决（Slice 11，ADR 0004 amendments）
...
2026-07-28 再复核：前三步按先后顺序依次执行，不互斥。词距和中西间距达到各自
上限后，若一行仍有余量，它们仍与其他有效间距一起获得相同的最终字距。一个
source 空格只算一次。CLREQ 明令排除的两类保持关闭：符号分离禁则规定的组合
内部，以及连接号、分隔号与其前后字符之间；项目另关闭破折号、省略号两侧。
...
其余标点 glue 作为项目扩展排最后档；风格分歧被设计为 `AdjustmentStylePolicy`
...
初次审计时在段落层面的特性完全缺失：`ParagraphStyle` 无 indent；首行行首开括号的
半宽缩减也是待实现的配套规则。

已实现：首行行宽收为较窄 + `LineBox.indent` 渲染偏移；开括号半宽缩减不需要
...
- **§6.2.1 段落调整（多段排版）**。已解决（Slice 22，2026-06-16）：引擎单段→
  ...
  breaker 不改动任何代码（喂正文宽 + 相对首行缩进）。拼音 `RubySpan.baseRange` 已进入
...
- **数字+前后缀符号 符号分离禁则**。已解决（2026-06-16）：CLREQ §符号分离禁则
  ...
  被实现为 `NumberSymbolCohesion.unbreakableRanges(text)`（clreq，含内部小数点/
...
- **中西自动间距按 Unicode `East_Asian_Spacing` 接入**。已解决（2026-07-28，
...
- **ASCII 点号行首禁则**。已解决（2026-07-12；2026-08-11 与 UAX #14 分层）：CLREQ 记录了中文横排
  ...
  `AttachedAsciiPointMarkImpossibleMeasureHang` 悬挂作为最终后备方案。宽度判定与 breaker 同源（含 ruby/
...
- **GB 式固定半宽标点**。已解决（ADR 0027）：「不可调整的标点包括：GB 式
  半字连接号、间隔号、分隔号，固定半个字宽」。被实现为 `PunctuationWidthPolicy.
...
- **连接号按字细分宽度**。已解决（ADR 0027 amendment）：短横线（–
  ...
  被实现为 `forcedHalfWidth` 对短横线无条件返回 true（grid 占位，覆盖 glyph
  advance）。
...
- **行长字号整数倍**。已解决（ADR 0028）：grid-first 的行长部分。向下取整
  ...
  对齐在容器内摆放正文（`GridBodyAlignment`），默认开、可旁路。纯汉字行经过取整后
  justify 余量变为零。
- **纵横对齐（grid）的行内凑整**：西文/数字前后用 0~1/2 em 弹性空白把混排
  行内内容也凑到整字位，grid-first 的**行内内容**部分（行长部分已由
  ADR 0028 解决），远期。
- **三个以上标点连续**时禁则的放宽**有意不做**（非缺失项）。CLREQ 第六节
...
缺失项 1–7 全部已解决（Slice 10–17）；行尾禁则（ADR 0026）已实现；`cjkDigit`
自动间距按边界字符独立接入（CLREQ 字母/数字之分）+ 数字符号分离禁则
