...
- **西文词距拉伸上限是加法 cap**：CLREQ「每个西文词距最大可以拉伸到
  半个汉字字宽」指最终宽度 ≤0.5em（绝对）；我们用「自然空格宽 +0.25em」
  （`Justifier.wordSpaceStretchEm`）。自然空格 >0.25em 时拉满会越界
  ≤~0.08em。绝对 cap 更准，但量级可忽略，且确定性 stub 每码点 1em
  （空格也 1em）会令绝对 cap 把 stub 的词距 headroom 归零、丧失整档
  测试覆盖——故保留加法。真字体渲染时影响极小。
