...
**西文音节断词（连字符换行）——延迟到平台，不在 core 自建。** CLREQ 通用规则
（§换行与断词连字 →「横排中混排的西文单词……在可使用连字符处之外，不得分隔
为两行」）**允许**西文在「可使用连字符处」（西文体例的音节断点）加连字符换行，
只禁止在别处硬拆。这是平台 line-break 能力（ICU / 系统 `BreakIterator`、OS
断词器，目标平台自带），延迟到 `tiqian-linebreak` 适配器接真实断行时由平台
提供，core 不自建。⚠️ 不要把 §纵横对齐 的「行尾强制断行（不依音节、不加
连字符）」当作简体默认——那是繁体取向的覆盖，该节自注「纵横对齐……简体中文
较为少见」。
