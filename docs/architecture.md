...
- `shaping/android-adapter`：Android `TextPaint` / platform glyph data；
...
HarfBuzz session，并调用同一个 `layout` 生成最大版心的预排结果。纯文本与受控语义 inline
...
- `shaping/*`：平台 shaping contract 及其实现。
