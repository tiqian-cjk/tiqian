...
`tiqian-web` 发布 ESM 包 `@tiqian/prose` 与 light-DOM `<tiqian-prose>`。服务器输出的
HTML 先保持可读，Kotlin/JS runtime 与字体准备完成后再逐段增强。原 `<p>`、链接、代码、强调、自定义
...
`tiqian-web-precompute` 是独立的 Node Kotlin/JS 工具入口。它从站点明确发布的字体文件建立
HarfBuzz session，并调用同一个 `tiqian-layout` 生成最大版心的预排结果。快照只是一份绑定 source、
typography、font 与 width 证据的 cache；live SSR HTML 始终是事实来源。浏览器只有在证据与几何
全部匹配时才采用快照，否则回到实时 Web pipeline。完整契约见
...
- `tiqian-web-precompute`：Node exact-font session、构建期 layout 入口与快照 wire；不拥有排版规则。
