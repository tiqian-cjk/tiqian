...
internal object UnicodeExtendedPictographicData {
    ...
    fun contains(codePoint: Int): Boolean {
        var low = 0
        var high = (UnicodeExtendedPictographicData.RANGES.size / 2) - 1
        while ((low <= high)) {
            val middle = ((low + high) shr (1))
            val base = middle * 2
            if ((codePoint < UnicodeExtendedPictographicData.RANGES[base])) {
                high = middle - 1
            } else {
                if ((codePoint > UnicodeExtendedPictographicData.RANGES[base + 1])) {
                    low = middle + 1
                } else {
                    return true
                }
