...
export async function createPrecomputer(options = {}) {
  ...
  return Object.freeze({
    ...
    async prepareParagraph(input = {}) {
      ...
      if (issue) return Object.freeze({ status: "unsupported", key, issue });
      ...
      try {
        ...
        const serialized = runtime.precomputePlainParagraph(
          fontSession.id,
          text,
          maxWidthPx,
        ...
        if (capabilityIssue) return Object.freeze({ status: "unsupported", key, issue: capabilityIssue });
        throw error;
      }
...
function buildSnapshotBundle(preparedParagraphs, options = {}) {
  ...
  if (entries.some((entry) => entry.status !== "prepared")) {
  ...
  if (entries.some((entry) => stableStringify(entry.renderFontFamilies) !== renderFamilySignature)) {
  ...
  const renderedEntries = entries.map((entry) => {
    const rendered = renderPreparedParagraphArtifact(entry.plan, entry.typography, { styleClassFor });
    return {
      ...entry,
      html: rendered.html,
      renderArtifactSha256: sha256(stableStringify(rendered.artifact)),
  ...
  const manifest = compactSnapshotManifest(renderedEntries, {
    schema: SNAPSHOT_SCHEMA,
    layoutRevision: LAYOUT_REVISION,
    renderRevision: RENDER_REVISION,
...
/**
 * Produces both the inert validation template and compact, first-paint-safe
 * paragraph artifacts. A server adapter may splice `entries[].html` into the
 * keyed source paragraphs only in the final HTML response, then inject
 * `initialStyle` and `template` into the document head. Client-navigation data
 * can keep the untouched source DOM as the readable runtime fallback.
 */
export function renderSnapshotBundle(preparedParagraphs, options = {}) {
  const { inertTemplate: _inertTemplate, ...bundle } = buildSnapshotBundle(
    preparedParagraphs,
    options,
  );
