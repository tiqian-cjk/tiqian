...
object ZhuyinParser {
    ...
    fun parse(reading: String): ZhuyinReading {
        if (reading.isEmpty()) return ZhuyinReading(emptyList(), ZhuyinTone.Yinping)
        if (reading[0] == NEUTRAL) {
            return ZhuyinReading(symbolsOf(reading.substring(1)), ZhuyinTone.Neutral)
        }
        val tone = when (reading.last()) {
            YANGPING -> ZhuyinTone.Yangping
            SHANG -> ZhuyinTone.Shang
            QU -> ZhuyinTone.Qu
            else -> ZhuyinTone.Yinping
        }
        ...
        return ZhuyinReading(symbolsOf(body), tone)
    }
